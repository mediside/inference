...
path ='../dataset/'
mask_3Dpath = '../segmentation_mask/'

def load_total_view(patient_name):
    anomalys = []
    view_mask = []
    for name in view_name:
        anomaly = np.load(cam_path+patient_name+ '_' + name+'.npy')
        mask = cv2.imread(maskfactory + patient_name + '_' + name+'_mask.png',0)
        mask[mask > 0] = 1
...
for name in tqdm(patient):
    ...
    print(mask_3Dpath + patientname +'_mask.nii.gz')
